...
const pidFile = "/app/DATA/workers/app/workerd.pid";
...
const startWorkerd = () => {
  ...
  exec(
    "/usr/local/bin/workerd serve /app/DATA/workers/config.capnp > /app/DATA/workers/workerd.log 2>&1 & echo $! > /app/DATA/workers/workerd.pid",
    (error, stdout, stderr) => {
      if (error) {
        ...
        exec(
          ...
          (error, stdout, stderr) => {
            if (error) {
              ...
              return res
                .status(500)
                .json({ error: "Failed to restart workerd service" });
            }

            console.log("Workerd restarted successfully");
            res.json({
              success: true,
              message: "Workerd service restarted successfully",
            });
          },
        );
      });
      ...
      exec(
        ...
        (error, stdout, stderr) => {
          if (error) {
            ...
            return res
              .status(500)
              .json({ error: "Failed to start workerd service" });
          }

          console.log("Workerd started successfully");
          res.json({
            success: true,
            message: "Workerd service started successfully",
          });
        },
      );
    }
